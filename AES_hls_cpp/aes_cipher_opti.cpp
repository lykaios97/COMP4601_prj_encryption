#include <ap_int.h>
#include <hls_stream.h>
#include <stdint.h>

typedef ap_uint<8> byte;
typedef ap_uint<32> word;

#define Nb 4
#define Nk 4
#define Nr 10

const byte sbox[256] = {
    
};

const byte Rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
};

byte gmul_fast(byte x, byte coeff) {
#pragma HLS INLINE
    byte xtime = (x << 1) ^ ((x & 0x80) ? 0x1b : 0);
    switch (coeff) {
        case 0x01: return x;
        case 0x02: return xtime;
        case 0x03: return xtime ^ x;
        default: return 0;
    }
}

void SubBytes(byte state[4][4]) {
#pragma HLS INLINE
    for (int i = 0; i < 16; i++) {
#pragma HLS UNROLL
        state[i % 4][i / 4] = sbox[state[i % 4][i / 4]];
    }
}

void ShiftRows(byte state[4][4]) {
#pragma HLS INLINE
    byte t;
    t = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = t;

    byte t0 = state[2][0], t1 = state[2][1];
    state[2][0] = state[2][2];
    state[2][1] = state[2][3];
    state[2][2] = t0;
    state[2][3] = t1;

    t = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
    state[3][0] = t;
}

void MixColumns(byte state[4][4]) {
#pragma HLS INLINE
    for (int c = 0; c < 4; c++) {
#pragma HLS UNROLL
        byte s0 = state[0][c];
        byte s1 = state[1][c];
        byte s2 = state[2][c];
        byte s3 = state[3][c];

        state[0][c] = gmul_fast(s0, 0x02) ^ gmul_fast(s1, 0x03) ^ s2 ^ s3;
        state[1][c] = s0 ^ gmul_fast(s1, 0x02) ^ gmul_fast(s2, 0x03) ^ s3;
        state[2][c] = s0 ^ s1 ^ gmul_fast(s2, 0x02) ^ gmul_fast(s3, 0x03);
        state[3][c] = gmul_fast(s0, 0x03) ^ s1 ^ s2 ^ gmul_fast(s3, 0x02);
    }
}

void AddRoundKey(byte state[4][4], const word roundKey[4]) {
#pragma HLS INLINE
    for (int j = 0; j < 4; j++) {
#pragma HLS UNROLL
        word k = roundKey[j];
        state[0][j] ^= byte((k >> 24) & 0xFF);
        state[1][j] ^= byte((k >> 16) & 0xFF);
        state[2][j] ^= byte((k >> 8) & 0xFF);
        state[3][j] ^= byte(k & 0xFF);
    }
}

word RotWord(word w) {
#pragma HLS INLINE
    return (w << 8) | (w >> 24);
}

word SubWord(word w) {
#pragma HLS INLINE
    word result;
    result.range(31, 24) = sbox[w.range(31, 24)];
    result.range(23, 16) = sbox[w.range(23, 16)];
    result.range(15, 8)  = sbox[w.range(15, 8)];
    result.range(7, 0)   = sbox[w.range(7, 0)];
    return result;
}

void KeyExpansion(const byte key[16], word w[44]) {
#pragma HLS INLINE off
    for (int i = 0; i < Nk; i++) {
        w[i] = (word(key[4 * i]) << 24) |
               (word(key[4 * i + 1]) << 16) |
               (word(key[4 * i + 2]) << 8) |
               word(key[4 * i + 3]);
    }
    for (int i = Nk; i < Nb * (Nr + 1); i++) {
        word temp = w[i - 1];
        if (i % Nk == 0)
            temp = SubWord(RotWord(temp)) ^ (word(Rcon[i / Nk]) << 24);
        w[i] = w[i - Nk] ^ temp;
    }
}

void Cipher(byte in[16], byte out[16], const word w[]) {
    byte state[4][4];
#pragma HLS ARRAY_PARTITION variable=state complete dim=0

    for (int i = 0; i < 16; i++)
#pragma HLS UNROLL
        state[i % 4][i / 4] = in[i];

    AddRoundKey(state, &w[0]);

    for (int round = 1; round < Nr; round++) {
#pragma HLS UNROLL
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, &w[round * Nb]);
    }

    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, &w[Nr * Nb]);

    for (int i = 0; i < 16; i++)
#pragma HLS UNROLL
        out[i] = state[i % 4][i / 4];
}
